...
/*   Updated: 2024/08/16 16:03:41 by drabarza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			get_envp(char **envp, t_envp_struct **envp_struct, char *line);
int				cut_remaining_line_on_pipes(
					t_command_line **command_line, char *remaining_line);
int				parse_substrings(
					char **remaining_line, t_command_line **command_line);
int				get_arguments_and_redirections(
					t_substring **substring, char **remaining_line,
					t_command_line **command_line);
int				get_redirections(char **remaining_line,
					t_substring *substring, t_command_line **command_line);
int				get_arguments(char **remaining_line,
					t_substring *substring, t_command_line **command_line);
int				is_pipe_latest_character(char **remaining_line);
int				is_pipe_first_character(char *remaining_line);
int				count_len_to_cut(char *remaining_line);
int				check_char_validity(
					char *remaining_line, int len_to_quote, int flag, int j);
unsigned int	count_angled_bracket(char *str);
// for init 
int				init_envp_struct(t_envp_struct **envp_struct);
int				init_native_argument_struct(t_native_argument **n_argument);
...
size_t			get_len_and_extract_with_single_quotes(
					char *str, char **extracted_line);
size_t			get_len_and_extract_between_double_quotes(
					char *str, char **extracted_line);
size_t			get_len_and_extract_until_next_quote(
					char *str, char **extracted_line);
size_t			get_len_and_extract_until_next_dollar(
					char *str, char **extracted_line);
size_t			get_len_and_extract_until_next_quote_or_dollar(
					char *str, char **extracted_line);
size_t			get_len_and_extract_until_next_separator(
					char *str, char **extracted_line);
size_t			get_len_and_extract_until_next_separator_first_dollar_included\
					(char *str, char **extracted_line);
size_t			get_len_and_extract_until_next_separator_dollar_excluded(char *str, char **extracted_line);
...
void			expand_string_after_dollar2(char *str, t_expanded_argument **exp_arguments,t_envp_struct *envp_struct, char **definitive_content);
void			complete_expand_content_of_redirections(
					char **str,
					t_command_line *command_line);
void			complete_expand_content_of_arguments(
					char **extracted_line, t_command_line *command_line);
size_t			simple_expand_content_of_redirections(
					char *str, char **extracted_line, t_command_line **command_line);
size_t			simple_expand_content_of_arguments(char *str, t_expanded_argument **exp_arguments, char **definitive_content, t_command_line **command_line);
void			expand_content_when_heredoc(
					char **str, t_envp_struct *envp_struct);
int				add_to_definitive_content(char **definitive_content, char *extracted_line);
void			build_exec_struct(t_exec_struct **exec_struct);
