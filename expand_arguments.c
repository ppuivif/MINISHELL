...
static size_t	extract_and_expand_content_of_arguments_on_split(char *content, \
char **extracted_line, t_command_line **command_line)
{
	size_t	len;

	len = simple_expand_content_on_split(&content[0], extracted_line, command_line);
//		special_treatment with $
	return (len);
}
...
static int	get_arguments_on_split(char *content, char **definitive_content, \
t_command_line **command_line)
{
	char	*extracted_line;
	int		len;

	len = (int)extract_and_expand_content_of_arguments_on_split(content, &extracted_line, \
	command_line);
	if (!extracted_line)
		return (-1);
	*definitive_content = ft_strdup(extracted_line);
...
t_command_line **command_line)
{
	...
	if (n_argument->content[0] == '$')
	{
		...
		{
			len = get_arguments_on_split(&n_argument->content[0], \
