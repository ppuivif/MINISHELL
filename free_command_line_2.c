...
static void	free_n_redirection(t_native_redirection **n_redirections)
{
	...
	while (ft_lst_size2(*n_redirections))
	{
		...
		free_and_null((*n_redirections)->content);
		free_and_null(*n_redirections);
		*n_redirections = tmp;
	}
}
...
static void	free_n_argument(t_native_argument **n_arguments)
{
	...
	while (ft_lst_size3(*n_arguments))
	{
		...
		free_and_null((*n_arguments)->content);
		free_and_null(*n_arguments);
		*n_arguments = tmp;
	}
}
...
static void	free_exp_redirection(t_expanded_redirection **exp_redirections)
{
	...
	while (ft_lst_size4(*exp_redirections))
	{
		...
		free_and_null(*exp_redirections);
...
static void	free_exp_argument(t_expanded_argument **exp_arguments)
{
	...
	while (ft_lst_size5(*exp_arguments))
	{
		...
		free_and_null((*exp_arguments)->content);
		free_and_null(*exp_arguments);
		*exp_arguments = tmp;
	}
}
