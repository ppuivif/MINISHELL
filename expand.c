...
//			expand_redirections((*command_line)->substrings, tmp1->n_redirections);
			remaining_line = tmp2->content;
			while (remaining_line)
			{
				expand_redirections(tmp1, tmp2, &remaining_line);
...
//	i = 0;
	if (n_redirection)
	{
		...
		{
			len_to_next_quote = strcspn(&n_redirection->content[1], "\'");
			extracted_line = ft_substr(&n_redirection->content[1], 0, len_to_next_quote);
//			n_redirection->content += len_to_next_quote + 2;
//			search_variables(extracted_line);
		}
		else if (n_redirection->content[0] == '\"')
		{
			len_to_next_quote = strcspn(&n_redirection->content[1], "\"");
			extracted_line = ft_substr(&n_redirection->content[1], 0, len_to_next_quote);
//			n_redirection->content += len_to_next_quote + 2;
...
//			n_argument->content += len_to_next_quote + 2;
//			search_variables(extracted_line);
		}
