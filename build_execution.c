...
void	build_exec_redirection_struct(t_expanded_redirection *exp_redirection, t_exec_subline **exec_subline, t_command_line **command_line, t_exec_struct **exec_struct)
{
	t_exec_redirection	*exec_redirection;

	if (init_exec_redirection_struct(&exec_redirection) == -1)
		error_allocation_exec_struct(exec_struct, command_line);
	(*command_line)->exit_code = open_and_check_file(exp_redirection, &exec_redirection);
	if ((*command_line)->exit_code != 0)//code to find when file couldn't be opened
			return ;
...
void	build_exec_argument_struct(t_expanded_argument *exp_argument, t_exec_subline **exec_subline, t_command_line **command_line, t_exec_struct **exec_struct)
{
